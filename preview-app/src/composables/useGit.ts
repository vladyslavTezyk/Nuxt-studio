...
export function useGit({ owner, repo, branch, token, authorName, authorEmail }: { owner: string, repo: string, branch: string, token: string, authorName: string, authorEmail: string }) {
  ...
  return {
    async fetchFile(path: string, { cached = false }: { cached?: boolean } = {}): Promise<GithubFile> {
      if (cached) {
      ...
      const res = await $api(`/repos/${owner}/${repo}/contents/${path}`).catch(() => null)
      if (cached) {
        gitFiles[path] = res
      }
      return res
